...
class GameField extends JPanel {
    ...
    public void aiTurn() {
        int randX, randY;
        for (randX = 0; randX < SIZE; randX++)
            for (randY = 0; randY < SIZE; randY++)
                if (isValidDot(randX, randY)) {
                    map[randY][randX] = PLAYER_DOT;
                    if (winCheck(PLAYER_DOT)) {
                        map[randY][randX] = AI_DOT;
                        return;
                    }
                    map[randY][randX] = EMPTY;
                }
        do {
